...
# NOTE there is a new performance package in here!!!!
...
#check your factors are not characters.
...
# in ggplot
ggplot(gotelli,aes(x=Latitude, y = Srich)) + geom_point() + geom_smooth()+facet_wrap(~Habitat)
ggplot(gotelli,aes(x=Latitude, y = Srich, col=Habitat)) + geom_point()+geom_smooth()

# plot residuals against the environmental variables

G <- resid(gotelli.glm, type='pearson') # extract the pearson residuals
plot(gotelli$Latitude~G)
boxplot(gotelli$as.factor(Habitat)~G) # fine 
boxplot(G~Habitat, data = gotelli) # slight issue 
...
Road.glm4 <- update(Road.glm2, .~. -D.WAT.COUR)
...
library(MASS)
#Remember to compare the residuals against explanatories
R <- resid(Road.nb2, type='pearson') # extract the pearson residuals
plot(Road$ OPEN.L~R)
plot(Road$ L.WAT.C~R)
plot(Road$ D.PARK~R)
...
# And Cooks distance
plot(spider.glm, which = 4) # This is dancing on the margin at (0.8). Logan is okay with it.
...
# graphically plot the Cooks distances
plot(bolger.glm, which = 4) # 19 is high but not 1. I'd still probably remove it but let's plough on
...
library(ggplot2)
ggplot(bolger, aes(x=PERSHRUB, y=RODENTSP)) +
  geom_point() +
