# *********************************************************************************
# This is the source script for Data Exploration and Visualisation                 *
# *********************************************************************************
# Jon Sadler Updated Feb 2022 and 
...
# Our focus will be on base graphics and Lattice (trellis) graphics; I'll produce additional code for using 
# ggplot as supplements for you to play with as well (for some things at least).

# **********************************************************
# PART ONE: Displaying frequencies and distributions of variables using histograms and barcharts
# **********************************************************
...
# **********************************************************
# *** PART 1.1: Barcharts  ****   
#Use datafile called RIKZ.txt. This is file listing the macroinvertebrate richness of 
# of 45 samples from 9 Dutch beaches (Zuur et al. Beginner's guide)
# We're going to create a barchart of the species richness on the nine beaches
# **********************************************************
# Load the data
...
# We are going to use the deer parasite data from Zuur's beginner's guide (Deer.csv);
...
dotchart(Deer$LCT, groups = as.factor(Deer$Sex)) # using as.factor as the sex column is an integer
...
ggplot(data=subset(Deer, !is.na(Sex)), aes(x = LCT, y = Farm)) +
  geom_point() + 
  facet_wrap(~ Sex, nrow = 1)
# NOTE - we have removed the NAs using !is.na and the subset function.
...
# Longer time series using dates and times......using the openair.csv file of NOx concentrations
...
# We are using base graphics in the main here and the data are stored as a matrix
...
yosemite <- as.matrix(read.csv(file.choose())) # datafile is called "yosemite.csv"
head(yosemite)
str(yosemite)
...
# now let's use some data with spatial attributes (lat, long) to do similar things
# We are going to use an example from the R Graphics Cookbook:
...
# filename = Riv_hydrol.csv; We are going to compare the Mean discharge (Cumecs) with the range 
# of discharge (Cumecs) with catchment area (Km2 = Fieldname = Carea)
...
# Sort by order largest to smallest (see - before sort field) so the largest circles are 
# plotted first
...
# The trouble with this approach is that the circles for catchment size are 
# proportional to the radius of the circle not the area of the circle
...
# fg and bg arguments set the colour of the colour of the symbols; setting bg = "white"
# gives the whilte outline
# the text() function adds labels to the circles. text(X axis, Y axis, Label)
...
# THIS IS FOR REFERENCE ONLY FOR THE TIME BEING UNTIL YOU BECOME MORE FAMILIAR WITH LOOPS

#*********************************************************************************
# PART 5: MULTIPLE PLOTS using LOOPS (code from Zuur et al. Beginners Guide to R)*
#*********************************************************************************

# We're using a dataset Owls.txt that is based on begging behaviour of owl chicks when their parents
# return to the next with chicks
# There are 27 nests in the study 

# The first thing you need to do is figure out what you are trying to achieve. Here we want 27 scatterplots
# of chick begging behaviour. So firstly we need the code to create one plot from one nest before we
# can GENERALISE the procedure and create the loop.
...
plot(x = Owls.ATV$ArrivalTime, y = Owls.ATV$NegPerChick,
     ...
     main =  "AutavauxTV")
# We need to repeat this a further 26 times to get the plot! You could paste it in 26 times and change the 
# nest names but that is a waste of effort
...
plot(x = Owls.i$ArrivalTime, y = Owls.i$NegPerChick,
     xlab = "Arrival Time", ylab = "Negotiation behaviour",
     main =  "AutavauxTV")
# but we still have the name in twice - so that lots of repeats. To avoid this we create a variable Nest.i
# that contains the name of the nest
Nest.i <- "AutavauxTV"
...
# Now we can start to create plots PDFs - using save PDF (or jpeg, or TIFF). We can automatically name the output
...
#*****************************************************************************
# 1. Use the Lattice and ggplot2 packages to plot depth v bioluminscence for each station. Read the help file to see how do this.
